...
var Router = (function () {
    ...
    Router.prototype.start = function () {
        ...
        router.get('/', function (req, res) {
            ...
            res.render('login', { title: title, errors: arrErrors });
        });
        // =====================
        // POST - login page ===
        // =====================
        ...
        router.get('/CreateAccount', function (req, res) {
            ...
            var strUserName = '', strPswd = '', arrErrors = [];
                                ...
                                fs.exists(thisfile.path, function (exists) {
                                    if (exists) {
                                        var writestream = gfs.createWriteStream({
                                            filename: req.session.user.username + "_pp",
                                            mode: 'w',
        ...
        router.post('/viewpage', function (req, res) {
            ...
            if (srch == '') {
                ...
                var query = ComicDoc.find({ $text: { $search: srch } }, { score: { $meta: "textScore" } });
            }
            query.where('isPublic').equals(true);
            ...
            query.exec(function (err, docs) {
                if (err) {
                    ...
                    if (req.body.order == 'relevance' && req.body.sort != 'desc')
                        docs.reverse();
                    console.log('Have comics: ' + docs);
                    ClearDownloadDirectory();
                    var count = 0;
        ...
        router.post('/playback', function (req, res) {
            ...
            UserDoc.findById(req.session.userId, function (err, user) {
                ...
                if (!user) {
                    console.log("USER NULL FOR SOME REASON");
                }
                else {
        ...
        router.get('/editpage', function (req, res) {
            ...
            var title, dialog1, dialog2, dialog3, dialog4;
        ...
        router.get('/confirmdelete', function (req, res, next) {
            ...
            res.render('confirmdelete', { title: title, ComicStripId: req.query.ComicStripId, ComicStripName: req.query.ComicStripName });
        });
